...
<script>
    ...
    function get_data(page){
        $("#loader").show();
        $.ajax({
            url:"{{ url('get_bike_rent') }}?page="+page,
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type:"POST",
            dataType:"JSON",
            success:function (data) {
                ...
                for(i in data.data){
                    ...
                    htmlData+='<td>'+data.data[i].bikes.company+'-'+data.data[i].bikes.plate+'</td>';
                    htmlData+='<td>'+data.data[i].trans_code+'</td>';
                    htmlData+='<td>'+data.data[i].trans_date+'</td>';
                    htmlData+='<td>'+data.data[i].other_details+'</td>';
                    htmlData+='<td>'+data.data[i].rider_amount+'</td>';
                    htmlData+='<td>'+data.data[i].vendor_amount+'</td>';
                    ...
                    htmlData+='<td>';
                    htmlData+=' <a  class="btn btn-danger btn-xs" href="javascript:void(0)" onclick="del_rec(\''+data.data[i].trans_code+'\', \'{{ url('bike_rent') }}/'+data.data[i].trans_code+'\')"><i class="fa fa-trash"></i> </a>';
                    htmlData+='</td>';
                    htmlData+='</tr>';
                }
